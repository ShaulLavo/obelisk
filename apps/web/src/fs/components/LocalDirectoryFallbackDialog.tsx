...
export const LocalDirectoryFallbackDialog: Component = () => {
	...
	const handleFilesSelected: JSX.EventHandlerUnion<
		HTMLInputElement,
		InputEvent
	> = async event => {
					...
					{singleMode() ? (
						<>
							<p class="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-[11px] text-amber-100">
								This browser only supports the temporary memory import. Load a folder to edit it for this session; the workspace will reset as soon as you reload and changes won't sync to disk.
							</p>
						...
						<>
							<p class="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-[11px] text-amber-100">
								Load the folder into memory for a one-off editing session (it resets the moment you reload), or copy it into browser storage (OPFS) to keep it across reloads. Neither option can write back to disk.
							</p>
							<p class="text-[11px] text-zinc-400">
