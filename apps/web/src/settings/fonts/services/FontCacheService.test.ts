	...
	it('property: cache size management with LRU eviction policy', async () => {
		await fc.assert(
			fc.asyncProperty(
				...
				fc.array(
					fc.record({
						name: fc.string({ minLength: 1, maxLength: 20 }).filter(name => 
				...
				async (fontMetadataList, cacheLimit) => {
					...
					const totalSize = uniqueFonts.reduce((sum, font) => sum + font.size, 0)
					...
					if (totalSize > cacheLimit) {
						...
						if (fontsToRemove.length > 0) {
							const remainingSize = totalSize - fontsToRemove.reduce((sum, fontName) => {
							...
							const removedFonts = uniqueFonts.filter(f => fontsToRemove.includes(f.name))
							const keptFonts = uniqueFonts.filter(f => !fontsToRemove.includes(f.name))
							
							if (removedFonts.length > 0 && keptFonts.length > 0) {
								// All removed fonts should be older than or equal to all kept fonts
								const oldestKeptTime = Math.min(...keptFonts.map(f => f.lastAccessed.getTime()))
								const newestRemovedTime = Math.max(...removedFonts.map(f => f.lastAccessed.getTime()))
	...
	it('property: LRU eviction maintains chronological order', async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.array(
					fc.record({
						name: fc.string({ minLength: 1, maxLength: 10 }).filter(name => 
							/^[a-zA-Z0-9-_]+$/.test(name)
						),
				...
				async (fontList) => {
					...
					for (let i = 1; i < sortedByAccess.length; i++) {
						expect(sortedByAccess[i].lastAccessed.getTime())
							.toBeGreaterThanOrEqual(sortedByAccess[i - 1].lastAccessed.getTime())
					}
					...
					if (removedFonts.length > 0 && remainingFonts.length > 0) {
						...
						const latestRemovedTime = Math.max(...removedFonts.map(f => f.lastAccessed.getTime()))
						const earliestRemainingTime = Math.min(...remainingFonts.map(f => f.lastAccessed.getTime()))
	...
	it('property: cache size calculation is accurate', async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.array(
					fc.record({
						name: fc.string({ minLength: 1, maxLength: 15 }).filter(name => 
							/^[a-zA-Z0-9-_]+$/.test(name)
						),
				...
				async (fontList) => {
					...
					const expectedTotalSize = uniqueFonts.reduce((sum, font) => sum + font.size, 0)
					...
					if (uniqueFonts.length === 0) {
						...
						const sumOfSizes = uniqueFonts.reduce((sum, font) => sum + font.size, 0)
